...
insert into chain (name) values ('sui');
insert into chain (name) values ('aptos');

create table profile (
    "id" bigserial primary key,
    "chain_asset_id" varchar(500) NOT NULL,
    ...
    constraint fk_profile_chain foreign key(chain_id) references chain(id)
);
...
create table post (
    ...
    constraint fk_post_profile foreign key(user_id) references profile(id),
    constraint fk_post_chain foreign key(chain_id) references chain(id)
);
